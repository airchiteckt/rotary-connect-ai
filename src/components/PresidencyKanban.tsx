...
import { Calendar, DollarSign, User, Plus, Edit, Trash2, Lightbulb, ClipboardList, CheckCircle2, Target } from 'lucide-react';
...
interface PresidencyKanbanProps {
  onStatsUpdate: () => void;
}

export default function PresidencyKanban({ onStatsUpdate }: PresidencyKanbanProps) {
  const { user } = useAuth();
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-lg font-semibold">Kanban Progetti Presidenza</h3>
          <p className="text-sm text-muted-foreground">
            Trascina i progetti tra le colonne per cambiare stato
          </p>
        </div>
        <Dialog open={showProjectForm} onOpenChange={(open) => {
          ...
          <DialogContent className="max-w-2xl">
            <DialogHeader>
              ...
              <DialogDescription>
                {editingProject ? 'Modifica i dettagli del progetto.' : 'Inserisci i dettagli del nuovo progetto.'}
              </DialogDescription>
            </DialogHeader>
            <ProjectForm 
